...
class NewCaloriesBarChart extends StatefulWidget {
  // List<DateTime>  DateOfOneWeekList;
  // Map DateOfOneWeekList = {
  //   'protein': ,
  //   'fat':this.fat,
...
class NewCaloriesBarChartState extends State<NewCaloriesBarChart> {
  ...
  void judgeDate({DateTime time}) {
    ...
    if (settingDay.isBefore(this.sundayDate) ||
        settingDay.compareTo(this.sundayDate) == 0) {
      //如果是1.14?在本周日之前 但不在周一15号之后
      if (settingDay.isAfter(this.mondayDate) ||
          settingDay.compareTo(this.mondayDate) == 0) {
        ...
        touchTooltipData: BarTouchTooltipData(
            ...
            getTooltipItem: (group, groupIndex, rod, rodIndex) {
              ...
              switch (group.x.toInt()) {
                case 0:
                  weekDay =
                      formatDate(this.mondayDate, [yyyy, '-', mm, '-', dd]);
                  break;
                case 1:
                  weekDay =
                      formatDate(this.tuesdayDate, [yyyy, '-', mm, '-', dd]);
                  break;
                case 2:
                  weekDay =
                      formatDate(this.wednesdayDate, [yyyy, '-', mm, '-', dd]);
                  break;
                case 3:
                  weekDay =
                      formatDate(this.thursdayDate, [yyyy, '-', mm, '-', dd]);
                  break;
                case 4:
                  weekDay =
                      formatDate(this.fridayDate, [yyyy, '-', mm, '-', dd]);
                  break;
                case 5:
                  weekDay =
                      formatDate(this.saturdayDate, [yyyy, '-', mm, '-', dd]);
                  break;
                case 6:
                  weekDay =
                      formatDate(this.sundayDate, [yyyy, '-', mm, '-', dd]);
                  break;
              }
