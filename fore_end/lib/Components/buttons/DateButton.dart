...
class DateSelect extends StatefulWidget with ValueableWidgetMixIn<int> {
  ...
  DateSelect(
    ...
    this.initTime =
        DateTime(this.initTime.year, this.initTime.month, this.initTime.day);
    this.widgetValue =
        new ValueNotifier<int>(this.initTime.millisecondsSinceEpoch);
    ...
    this.beginTime =
        DateTime(this.beginTime.year, this.beginTime.month, this.beginTime.day);
    this.lastTime = lastTime ?? DateTime(2021, 12, 1);
    this.lastTime =
        DateTime(this.lastTime.year, this.lastTime.month, this.lastTime.day);
  }
...
class DateSelectState extends State<DateSelect> with ValueableStateMixIn {
          ...
          GestureDetector(
              onTap: () async {
                final DateTime date = await showDatePicker(
                    context: context,
                    locale:
                        Locale(CustomLocalizations.of(context).nowLanguage()),
                    ...
                    builder: (context, child) {
                      return Theme(
                          data: ThemeData.from(
                            ...
                            primary:
                                MyTheme.convert(ThemeColorName.HightLightIcon),
                            primaryVariant:
                                MyTheme.convert(ThemeColorName.DarkIcon),
                            secondary:
                                MyTheme.convert(ThemeColorName.HighLightText),
                            secondaryVariant:
                                MyTheme.convert(ThemeColorName.HighLightButton),
                            surface: MyTheme.convert(
                                ThemeColorName.ComponentBackground),
                            background:
  ...
  void _modifyDat({int day = 1}) {
    ...
    if (dt.compareTo(widget.lastTime) <= 0 &&
        dt.compareTo(widget.beginTime) >= 0) {
