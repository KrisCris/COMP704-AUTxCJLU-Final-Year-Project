...
class BodyWeightChartState extends State<BodyWeightChart> {
  ...
  Widget build(BuildContext context) {
    SfCartesianChart chart = SfCartesianChart(
      ...
      primaryYAxis: NumericAxis(
          majorGridLines: MajorGridLines(width: 1, color: Colors.blueGrey),
  ...
  void repaintData() {
    ...
    if (u.isOffline) {
      ...
      for (Map m in obj) {
        bodyChanges.add(BodyChangeLog(
            time: m['time'], weight: m['weight'], height: m['height']));
      }
      if (mounted) {
        setState(() {});
      }
    ...
    Requests.getWeightTrend(context, {
      ...
      "token": u.token,
      "begin":
          (now.add(Duration(days: -60)).millisecondsSinceEpoch / 1000).floor(),
      ...
      for (Map m in value.data['data']['trend']) {
        ...
        if (oneDay == null) {
          ...
          if (oneDay.year == tm.year &&
              oneDay.month == tm.month &&
              oneDay.day == tm.day) {
            //取同一天里靠后的那次数据
      ...
      if (oneDay != null && oneWeight != null) {
        bodyChanges.add(BodyChangeLog(
            time: oneDay.millisecondsSinceEpoch, weight: oneWeight, height: 0));
      }
      if (mounted) {
        setState(() {});
      }
